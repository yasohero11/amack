...
@section("js")
    ...
    <script>
        ...
        async function createAuthor() {
           ...
           await AFetch({
                ...
                onResponse: (response) => {
                    closeModel()
                    getAuthor()
                    popupAlert.showSuccessAlert("Author Created")
                },
       ...
       async function updateAuthor() {
           let loading = popupAlert.showLoadingAlert("Updating Author ...")
            hideInvalidMessages()
            const data = new FormData(form);
            await AFetch({
                method: "POST",
                body: data,
                url: "{{route("update.authors",["id"=> ":id"])}}".replace(':id', selectedItem),
                onResponse: (response) => {
                    closeModel()
                    getAuthor()
                    popupAlert.showSuccessAlert("Author Updated")
                },
                onError : async(response) => {
                    console.log(response)
                    if(response.status === 422)
        ...
        async function deleteAuthor(id) {
            ...
            await AFetch({
                ...
                onResponse: (response) => {
                    selectedItem = -1
                    getAuthor()
                    popupAlert.showSuccessAlert("Author Deleted")
                },
                onError : async(response) => {
                    console.log(response)
                    if(response.status === 422)
...
@section('content')
    ...
    <dialog id="MiniDialog">
        <form method="dialog" id="form">
            ...
            <div class="mb-3">
                <label for="password" class="form-label">Author Password</label>
                <input type="password" class="form-control" name="password" id="password" placeholder="Author Password">
                <div class="name-invalid-feedback invalid-feedback">
                    Please choose a username.
                </div>
            </div>
            ...
            <footer>
                <menu style="padding: 0px">
                    <div class="ms-auto d-flex justify-content-end">
                        ...
                        <button type="button" onclick="selectedItem === -1? createAuthor() : updateAuthor()"
                    ...
                    <div class="card-body px-0 py-0">
                        <div class="table-responsive p-0">
                            <table id="table" class="table align-items-center justify-content-center mb-0">
                                ...
                                @include("General.parts.dashboard.tables.authors")
                                </tbody>
                            </table>
                        </div>
                    </div>
